...
| 插件 | 目录 | 版本 | 主要功能 |
| --- | --- | --- | --- |
| 字幕匹配 | `plugins.v2/subtitlemanualupload` | 0.1.82 | 字幕搜索、下载、上传、匹配、改名、调轴、入库自动处理 |
| AI字幕生成(联动版) | `plugins.v2/autosubv3` | 3.5.59 | 音轨识别、字幕提取、AI 翻译、任务队列、联动字幕匹配 |
...
- 联邦前端已在真实 MoviePilot-Frontend 宿主中完成移动端适配，窄屏下搜索、详情、历史和弹窗可用，桌面布局通过回归测试保持原有结构。
...
- 支持与“字幕匹配”互相联动。
- 联邦任务页已在真实 MoviePilot-Frontend 宿主中完成移动端适配，窄屏下筛选、批量选择和重跑弹窗可用，桌面工具栏通过回归测试保持横向布局。
