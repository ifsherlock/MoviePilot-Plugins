...
class MonitorService:
    ...
    def check_subtitlemanualupload_auto_transfer_enabled(plugin_manager_getter, logger) -> bool:
        ...
        try:
            ...
            return False
        plugin = (running_plugins.get("SubtitleManualUpload")
                  or running_plugins.get("subtitlemanualupload")
                  or running_plugins.get("SubtitleManualUploadLocal")
                  or running_plugins.get("subtitlemanualuploadlocal"))
        if not plugin:
            for candidate in running_plugins.values():
                if candidate.__class__.__name__ in ("SubtitleManualUpload", "SubtitleManualUploadLocal"):
                    plugin = candidate
                    break
        if not plugin:
            return False
