...
class MediaTargetResolver:
    ...
    @staticmethod
    def _history_identity(history: Any):
        source, media_id = resolve_media_identity(history)
        if source:
            return source, media_id
        legacy_tmdb = getattr(history, "tmdbid", None)
        if legacy_tmdb:
            return MediaSource.TMDB, str(legacy_tmdb).strip()
        legacy_douban = str(getattr(history, "doubanid", "") or "").strip()
        if legacy_douban:
            return MediaSource.Douban, legacy_douban
        return None, None

    @staticmethod
    def _legacy_identity(*, tmdb_id: Any = None, douban_id: Any = None):
        tmdb = str(tmdb_id or "").strip()
        if tmdb and tmdb != "0":
            return MediaSource.TMDB, tmdb
        douban = str(douban_id or "").strip()
        if douban:
            return MediaSource.Douban, douban
        return None, None
    ...
    def build_entry_from_history(self, history: Any) -> Optional[Dict[str, Any]]:
        ...
        media_source, media_id = self._history_identity(history)
        tmdb_id = self._safe_int(media_id, 0) if media_source == MediaSource.TMDB else 0
        douban_id = media_id if media_source == MediaSource.Douban else ""
        media_key = build_media_key(media_source, media_id) or self._hash_text(
    ...
    def entries_from_transfer_event(self, event_data: Dict[str, Any]) -> List[Dict[str, Any]]:
        ...
        media_source, media_id = resolve_media_identity(mediainfo)
        tmdb_id = self._safe_int(media_id, 0) if media_source == MediaSource.TMDB else 0
        douban_id = media_id if media_source == MediaSource.Douban else ""
    ...
    def targets_for_media(
        ...
        source, identity = resolve_media_identity(
            media_source=media_source,
            media_id=media_id,
        )
        if not source:
            source, identity = self._legacy_identity(tmdb_id=tmdb_id, douban_id=douban_id)
        clean_tmdb_id = self._safe_int(identity, 0) if source == MediaSource.TMDB else 0
        clean_title = self._normalize_text(title)
        clean_year = self._normalize_text(year)
        clean_douban_id = identity if source == MediaSource.Douban else ""
        ...
        for entry in self._load_local_entries(allow_stale=True):
            if clean_type and entry.get("media_type") != clean_type:
                continue
            entry_source, entry_id = resolve_media_identity(entry)
            if source and (entry_source != source or entry_id != identity):
                continue
            if not source and clean_title and entry.get("title") != clean_title:
                continue
        ...
        media = media_groups[0] if media_groups else {
            "id": build_media_key(source, identity) or self._hash_text(
                f"{clean_type}|{clean_title}|{clean_year}"
            ),
            "media_source": source.value if source else "",
            "media_id": identity or "",
