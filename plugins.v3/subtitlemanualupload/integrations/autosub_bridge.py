...
class AutoSubBridge:
    ...
    def autosub_plugin(self) -> Tuple[Any, str]:
        ...
        plugin = (running_plugins.get("AutoSubv3")
                  or running_plugins.get("autosubv3")
                  or running_plugins.get("AutoSubv3Local")
                  or running_plugins.get("autosubv3local"))
        if not plugin:
            for candidate in running_plugins.values():
                if candidate.__class__.__name__ in ("AutoSubv3", "AutoSubv3Local"):
                    plugin = candidate
                    break
        if not plugin:
