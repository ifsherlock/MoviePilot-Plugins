...
export const autoTransferQueue = {
  ...
  tasks: [
    ...
    {
      id: 'queue-extreme-long-failure',
      title: '极端移动端队列测试：非常非常长的电影标题 With Multiple Aliases And UHD Release Notes',
      provider: 'opensubtitles',
      status: 'failed',
      message: '极端失败原因：源站返回 Cloudflare challenge，随后备用 API 返回 429 rate limit，最终未找到可下载字幕，需要用户稍后重试或手动上传字幕包。',
      created_at: '2026-07-07 02:33:00',
    },
  ],
}
...
export const onlineResults = [
  ...
  {
    provider: 'opensubtitles',
    result_id: 'os-extreme-long-title-manual',
    title: 'EXTREME MOBILE RESULT - The.Longest.Mobile.Layout.Regression.Title.2026.S01E03.INTERNAL.MULTi.2160p.WEB-DL.DDP5.1.Atmos.HDR10.Release.Group.With.An.Absurdly.Long.Subtitle.Name',
    language: 'English',
    language_category: 'english',
    format: 'zip',
    season: 1,
    ...
    score: 64,
    downloadable: false,
    page_url: 'https://example.invalid/opensubtitles/extreme-long-manual-only',
    note: '极端在线结果说明：这是一个需要手动打开源站验证的结果，标题、备注和匹配说明都故意很长，用于验证移动端结果卡片不会横向撑破。',
    match_detail: '弱匹配：只命中别名和年份，没有 TMDB ID，且字幕包包含多个版本，需要用户在详情里确认。',
    identity_status: 'weak',
  },
]
...
export const aiTasks = {
  ...
  tasks: [
    ...
    {
      task_id: 'ai-task-extreme-long-error',
      target_id: 'target-s01e03',
      target_label: 'S01E03 · 极端失败原因与长路径',
      video_name: '极端移动端 AI 状态测试 S01E03 With A Very Long Alias',
      video_file: `/mnt/media/TV/极端移动端 AI 状态测试/Season 01/${subtitleTargets[2].basename}.mkv`,
      status: 'failed',
      status_label: '失败',
      active: false,
      source: 'matched_external',
      source_label: '外挂字幕',
      source_asset_name: `${subtitleTargets[2].basename}.English.Forced.And.Commentary.Extremely.Long.Source.Name.ass`,
      output_name: `${subtitleTargets[2].basename}.ai.zh.srt`,
      add_time: '2026-07-07 02:01:00',
      complete_time: '2026-07-07 02:08:00',
      message: '极端失败原因：ASR 提取成功，但翻译模型连续 3 次返回非 JSON 内容，最后一次响应包含 1200 字解释文本和半截字幕数组；需要在移动端展开详情后仍能完整阅读并看到重试入口。',
    },
  ],
}
...
export const autoSubTasks = {
  ...
  tasks: [
    ...
    {
      task_id: 'autosub-extreme-pending-batch',
      video_name: '极端移动端任务卡片测试：超长标题、超长路径、超长输出文件名和等待队列状态',
      video_file: '/mnt/media/TV/Extremely Long AutoSub Mobile Stress Case/Season 01/Extreme.Mobile.Task.Card.Layout.Regression.S01E04.2160p.NF.WEB-DL.DDP5.1.Atmos.DV.HDR.HEVC-LongGroupName/Extreme.Mobile.Task.Card.Layout.Regression.S01E04.2160p.NF.WEB-DL.DDP5.1.Atmos.DV.HDR.HEVC-LongGroupName.mkv',
      status: 'pending',
      status_label: '排队中',
      active: true,
      source: 'asr',
      source_label: '音轨 ASR',
      resolved_source_label: '音轨 ASR · 第 2 音轨 English Commentary',
      output_name: 'Extreme.Mobile.Task.Card.Layout.Regression.S01E04.2160p.NF.WEB-DL.DDP5.1.Atmos.DV.HDR.HEVC-LongGroupName.ai.zh-Hans.forced-commentary.srt',
      add_time: '2026-07-07 02:35:00',
      complete_time: '',
      message: '等待前序任务释放 GPU 队列；移动端批量选择后需要保持主操作可点击，长说明不能挤压按钮。',
    },
  ],
}
