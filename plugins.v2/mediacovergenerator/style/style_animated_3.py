...
def create_style_animated_3(library_dir, title, font_path, font_size=(170,75), font_offset=(0,40,40),
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            logger.info(f"角标绘制检查 - style=animated_3, show_item_count={show_item_count}, item_count={item_count}, badge_style={badge_style}")
            if not show_item_count:
                logger.info(f"角标跳过 - style=animated_3, 原因: show_item_count=False (未启用角标)")
            elif item_count is None:
                logger.info(f"角标跳过 - style=animated_3, 原因: item_count=None (未获取到媒体数)")
