...
def create_style_animated_2(
    ...
    try:
        ...
        if not all_posters:
            logger.warning("style_animated_3 未找到素材图�?)
            return False
        ...
        with tempfile.TemporaryDirectory() as tmp_dir:
            ...
            for f in range(total_frames):
                ...
                # 取消帷幕动画（不再根�?bg_motion_mode 切换�?                # 保留固定斜切布局，仅做新旧画面渐变切�?                panel_mix_t = _ease_in_out_sine(local)
                ...
                # 动态边缘阴�?                edge_shadow = Image.new("RGBA", (target_w, target_h), (0, 0, 0, 0))
                ...
                # 标题固定，不做左右位移动�?                text_mix = _blend_rgba(prepared_text[idx], prepared_text[nxt], panel_mix_t)
