...
def create_style_static_2(image_path, title, font_path, font_size=(170,75), font_offset=(0,40,40), blur_size=50, color_ratio=0.8, resolution_config=None, bg_color_config=None, item_count=None, show_item_count=False, badge_style='badge', badge_size_ratio=0.12):
    try:
        ...
        # 绘制角标
        logger.info(f"角标绘制检查 - style=static_2, show_item_count={show_item_count}, item_count={item_count}, badge_style={badge_style}")
        if show_item_count and item_count is not None:
            logger.info(f"角标绘制 - style=static_2, item_count={item_count}, badge_style={badge_style}")
            try:
            ...
            logger.info(f"角标跳过 - style=static_2, 原因: show_item_count={show_item_count}, item_count={item_count}")
