...
- 支持在线字幕搜索：剧集可搜索本季/选中集字幕包，单集可点放大镜搜索；默认来源为 SubHD、Zimuku，射手网(伪) 需配置 API Key 后启用。
- 在线字幕下载后会进入同一个匹配预览流程，确认语言后缀、集数和文件名后再写入，不会直接落盘。
- 自动搜索失败或源站需要验证时，弹窗提供已启用字幕站的手动搜索链接，可下载后回到本页手动上传。
...
- 手动开启 MoviePilot 系统代理时，会把 `socks5h` 自动转换为 CloakBrowser/Playwright 支持的 `socks5`。
- 插件设置可维护三个字幕站根地址和射手网(伪) API Key，域名、反代地址或 API 入口变化时无需更新插件版本。
...
- 射手网(伪) 默认不参与自动搜索；配置 API Key 后优先访问 `https://api.assrt.net` 官方接口，未配置时仅在手动勾选后尝试网页仿真或提供手动搜索链接。
