...
- 支持在线字幕搜索：剧集可搜索本季/选中集字幕包，单集可点放大镜搜索；来源包括 SubHD、Zimuku、ASSRT。
...
- SubHD 优先自动搜索和下载；若源站要求公众号验证码，需要使用手动搜索链接处理。
- Zimuku 遇到网站防火墙时会尝试调用 MoviePilot OCR 识别验证码，失败时降级为手动搜索。
- ASSRT 不强制配置 Token；未配置时默认提供手动搜索链接，配置 Token 后才尝试 API 自动搜索。
