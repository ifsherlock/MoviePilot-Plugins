...
- 优化移动端长文件名和路径展示，剧集标题、文件名和外挂字幕名默认单行省略，避免卡片被长文本撑满。
- 路径默认弱化并收纳为一行，完整路径仍可在展开详情中查看，保留排错信息但提升手机端扫读效率。
- 保持桌面详情行布局不变，并通过真实 MoviePilot-Frontend 宿主截图回归验证。

## v0.1.82 更新

- 二次优化移动端剧集详情，新增剧集移动卡片和底部批量操作栏，搜索、上传、在线字幕和 AI 操作更适合触控。
- 上传、在线字幕、AI 状态和入库自动队列弹窗在移动端改为 sheet 结构，标题、正文和底部操作分层，长路径、长结果和失败原因不再横向溢出。
- 精简 AI字幕生成(联动版) 未启用提示为“请安装 AI字幕生成(联动版) 以启用相关功能”，避免重复文案。
- 已在真实 MoviePilot-Frontend 宿主验证 390px、430px、768px、1024px、1440px 视口和浅色/深色主题，桌面详情行与在线字幕侧栏布局保持不变。

## v0.1.81 更新

- 适配真实 MoviePilot-Frontend 宿主下的移动端布局，搜索页、剧集详情、匹配历史、上传弹窗、在线字幕弹窗和 AI 状态弹窗在窄屏下不再横向溢出。
- 移动端样式限定在窄视口断点内，1024px 和 1440px 桌面视口已通过回归测试，保留媒体多列、剧集密集行和在线字幕侧栏布局。
- Playwright 真实宿主测试使用假元数据和本地联邦 `remoteEntry.js`，覆盖浅色/深色主题、390px/430px/768px/1024px/1440px 视口和核心字幕工作流。
