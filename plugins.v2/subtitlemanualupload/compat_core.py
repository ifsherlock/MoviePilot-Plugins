...
def install_compat_core_methods(cls) -> None:
    cls._host_module_value = classmethod(host_module_value)
    cls._ok = ok
    cls._safe_int = staticmethod(safe_int)
    cls._normalize_text = staticmethod(normalize_text)
    cls._hash_text = staticmethod(hash_text)
    cls._brief_ids = classmethod(brief_ids)
    cls._decode_preview_bytes = staticmethod(decode_preview_bytes)
    cls._normalize_auto_transfer_subtitle_strategy = classmethod(
        lambda owner_cls, value: normalize_auto_transfer_subtitle_strategy(value)
    )
    cls._check_online_rate_limit = check_online_rate_limit_compat
    cls._entry_path_is_valid = classmethod(entry_path_is_valid)
    cls._entry_filesystem_signature = classmethod(
        lambda owner_cls, entry: target_entry_filesystem_signature(entry, normalize_text=owner_cls._normalize_text)
    )
    cls._timestamp_iso = staticmethod(timestamp_iso)
    cls._set_rar_dependency_status = lambda owner, state, message: setattr(
        owner,
        "_rar_dependency_status",
        type(owner)._archive_dependency_service().dependency_status(state, message),
    )
    cls._prepare_rar_dependency = lambda owner: type(owner)._archive_dependency_service(
        owner._set_rar_dependency_status
    ).prepare_rar_dependency()
    cls._normalize_language_suffix = classmethod(lambda owner_cls, value: normalize_language_suffix(value))
    cls._detect_language_profile = classmethod(
        lambda owner_cls, file_name, raw_bytes: detect_language_profile(file_name, raw_bytes, owner_cls._subtitle_exts)
    )
    cls._extract_episode_hint = classmethod(
        lambda owner_cls, file_name: target_extract_episode_hint(file_name, safe_int=owner_cls._safe_int)
    )
    cls._media_type_text = classmethod(lambda _owner_cls, value: target_media_type_text(value))
    cls._poster_url = classmethod(poster_url)
    cls._cache_loaded_at = classmethod(cache_loaded_at)
    cls._json_clone = staticmethod(json_clone)
    cls._timeline_task_summary = staticmethod(timeline_task_summary)
    cls._autosub_task_summary = staticmethod(bridge_autosub_task_summary)
    cls._entry_matches_keyword = classmethod(
        lambda owner_cls, entry, keyword: target_entry_matches_keyword(
            entry,
            keyword,
            normalize_text=owner_cls._normalize_text,
        )
    )
    cls._tmdb_detail_payload = classmethod(tmdb_detail_payload)
    cls._tmdb_aliases = classmethod(
        lambda _owner_cls, *values: target_tmdb_aliases(*values, extract_title_aliases_func=extract_title_aliases)
    )
    cls._apply_tmdb_detail = classmethod(lambda _owner_cls, target, detail: target_apply_tmdb_detail(target, detail))
    cls._is_stream_path = classmethod(is_stream_path)
    cls._is_upload_file = staticmethod(lambda value: isinstance(value, UploadFile))
    cls._timeline_result_blocks_auto_write = staticmethod(writer_timeline_result_blocks_auto_write)
    cls._timeline_rejection_message = staticmethod(writer_timeline_rejection_message)
    cls._subtitle_backup_path = classmethod(subtitle_backup_path)
